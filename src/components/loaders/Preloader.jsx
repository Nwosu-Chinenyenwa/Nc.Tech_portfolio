...
function Preloader({ children, preloaderSettings }) {
  ...
  return (
    <div className={`preloader-content-wrapper`}>
      ...
      {shouldShowContent && <PreloaderContent>{children}</PreloaderContent>}
    </div>
  );
}
  ...
  return (
    <div className={`preloader-window ${hiddenClass}`}>
      <div className={`preloader-window-content`}>
        <PacMan variant={PacMan.ColorVariants.LOADER} hidden={isPacManHidden} />
...
function PreloaderWindowInfo({ hidden, setDidLoadLogo }) {
  ...
  React.useEffect(() => {
    if (hidden) {
      ...
      const timer = setTimeout(() => setIsHidden(false), 600);
...
<div className="ai-matrix-loader">
  ...
  <div className="glow"></div>
      ...
      <div className="loadcenter">
        <h1 className="loadh1">Nc.Tech...</h1>
      </div>
    </div>
  );
}
